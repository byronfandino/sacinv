...
function validarCampos(){
    ...
    const expresion="^[A-ZÑa-züñáéíóúÁÉÍÓÚÜ'° ]{4,50}$";
        ...
        if (window.location.pathname.includes('/editar')){
            ...
            inputNombre.addEventListener('input', e => {
                ...
                if(labelError && labelError.textContent != ''){
                    ocultarError(inputNombre, iconoError);
                }
            });
            ...
            inputNombre.addEventListener('input', e => {
                ...
                let existeRegistro = arrayNombres.includes(stringFormateado);
                ...
                if(expresionRegular && !existeRegistro){
                    ...
                    mostrarOcultarSugerencias(labelSugr, 'Este método de pago ya se encuentra registrado', true);
                    // Deshabilitamos el botón
                    estadoBoton(btnSubmit, false);
                }

                let coincidencia = stringNombres.includes(stringFormateado);
...
async function obtenerRegistrosAPI(){
    try {
        ...
        const url='http://192.168.18.120:3000/metodo-pago/api';
        ...
        Swal.fire({
            icon:'error',
            title:'Error',
            text: 'No fue posible cargar los registros de la base de datos. Detalle Técnico del Error:' + error
        });
        return false;
    }
}
...
function mostrarRegistrosAPI(registros, evento = 'DOM', valor = ''){
    ...
    if (registros){
        ...
        registros.forEach( registro => {
            ...
            const {MP_Nombre} = registro;
            if(evento === 'DOM' && valor === ''){
    
                // Almacenamos en el arreglo global 'arrayNombres' todos los nombres para usarlos en otra función
                arrayNombres.push(formatearTexto(MP_Nombre));
            ...
            if(evento === 'input' && formatearTexto(MP_Nombre).includes(valor)){
                if(tabla){
                    crearRegistro(registro);
                }
            }
    ...
    stringNombres=arrayNombres.toString();
}
...
function crearRegistro(registro){
    const {MP_Id, MP_Nombre, MP_Status} = registro;

    const tdNombre = document.createElement('TD');
    tdNombre.classList.add('tbody__td--nombre');
    tdNombre.dataset.idItem = MP_Id;
    tdNombre.textContent = MP_Nombre;
    ...
    linkModificar.setAttribute('href', `/metodo-pago/editar?id=${MP_Id}`);
    ...
    linkHistorial.setAttribute('href', `/historial?id=${MP_Id}`);
