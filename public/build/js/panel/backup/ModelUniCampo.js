...
export function mostrarRegistrosAPI(evento = 'DOM', valor = ''){
    ...
    if (globales.registrosAPI){
        
        globales.registrosAPI.forEach( registro => {
            
            if (globales.llaves.length === 0){
                globales.llaves = Object.keys(registro);
            }
            ...
            if(evento === 'DOM' && valor === ''){
                globales.arrayDescripcion.push(formatearTexto(valorCampo));
                if (tabla){
    ...
    globales.stringDescripcion="";
    globales.arrayDescripcion.forEach(nombre => {
    globales.stringDescripcion += nombre.toString() + ",";
    });
