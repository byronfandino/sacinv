let registros;
let nombreCategorias=new Array();
    ...
    if(inputNombre){
        ...
        if (window.location.pathname.includes('/editar')){
            ...
            inputNombre.addEventListener('input', e => {
                ...
                let existeRegistro = nombreCategorias.includes(stringFormateado);
                ...
                if(expresionRegular && !existeRegistro){
                    ...
                    mostrarOcultarSugerencias(labelSugr, 'Esta categoría ya se encuentra registrada', true);
                    // Deshabilitamos el botón
                    estadoBoton(btnSubmit, false);
                }

                let coincidencia = stringCategorias.includes(stringFormateado);
...
async function obtenerRegistrosAPI(){
    try {
        ...
        const url='http://192.168.18.120:3000/categoria/api';
...
function mostrarRegistrosAPI(registros, evento = 'DOM', valor = ''){
    ...
    if (registros){
        ...
        registros.forEach( registro => {
            ...
            if(evento === 'input' && formatearTexto(Ctg_Descripcion).includes(valor)){
                if(tabla){
                    crearRegistro(registro);
                }
            }
    ...
    stringCategorias=nombreCategorias.toString();
}
...
function crearRegistro(registro){
    const {Ctg_Id, Ctg_Descripcion, Ctg_Status} = registro;

    const tdNombre = document.createElement('TD');
    tdNombre.classList.add('tbody__td--nombre');
    tdNombre.dataset.idItem=Ctg_Id;
    tdNombre.textContent = Ctg_Descripcion;
    ...
    linkModificar.setAttribute('href', `/categoria/editar?id=${Ctg_Id}`);
    ...
    linkHistorial.setAttribute('href', `/historial?id=${Ctg_Id}`);
