...
function validarCampos(){
    ...
    const expresion="^[A-ZÑa-züñáéíóúÁÉÍÓÚÜ'°_]{5,50}$";
    const msg="Solo debe contener letras y/o guión bajo ( _ ), mayor a 4 caracteres";
        ...
        if (window.location.pathname.includes('/editar')){
            ...
            inputNombre.addEventListener('input', e => {
                ...
                if(expresionRegular && !existeRegistro){
                    ...
                    mostrarOcultarSugerencias(labelSugr, 'Este registro ya se encuentra en la base de datos', true);
...
async function obtenerRegistrosAPI(){
    try {
        ...
        const url=`http://192.168.18.120:3000/${tabla}/api`;
...
function mostrarRegistrosAPI(registros, evento = 'DOM', valor = ''){
    ...
    if (registros){
        ...
        registros.forEach( registro => {
            ...
            if(evento === 'input' && formatearTexto(Tb_Nombre).includes(valor)){
                if(tabla){
                    crearRegistro(registro);
                }
            }
...
function crearRegistro(registro){
    const {Tb_Id, Tb_Nombre} = registro;

    const tdNombre = document.createElement('TD');
    tdNombre.classList.add('tbody__td--nombre');
    tdNombre.dataset.idItem = Tb_Id;
    tdNombre.textContent = Tb_Nombre;
    ...
    linkModificar.setAttribute('href', `/${tabla}/editar?id=${Tb_Id}`);
    ...
    linkHistorial.setAttribute('href', `/historial?id=${Tb_Id}`);
