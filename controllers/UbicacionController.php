<?php

namespace Controllers;

use MVC\Router;
use Model\Ubicacion;

class UbicacionController{

    public static function listarUbicaciones (){    
        if(!isset($_SESSION['nombre'])){
            header('Location: /');
        }
        
        $registros=Ubicacion::all('Ubicacion_Nombre');
        echo json_encode($registros);
    }

    public static function index(Router $router){
        
        if(!isset($_SESSION['nombre'])){
            header('Location: /');
        }

        $ubicacion = new Ubicacion();
        $alertas = [];

        if ($_SERVER['REQUEST_METHOD'] === 'POST'){
            
            //1. Primero sincronizamos los datos
            $ubicacion->sincronizar($_POST);
            
            //2.Validamos si no hay errores en la digitación del usuario
            $alertas = $ubicacion->validar();

            //3. Si hay alertas se termina la ejecución del código
            if(empty($alertas)){
                
                //4. Procedemos a verificar que no exista en la base de datos
                $existe = Ubicacion::where('Ubicacion_Nombre', trim($ubicacion->Ubicacion_Nombre));

                if($existe){
                    Ubicacion::setAlerta('error-ubicacion', 'nombre', 'Esta ubicacion ya existe en la base de datos');
                }else{
                    
                    // Si el registro no existe en la base de datos se procede a guardar
                    $resultado = $ubicacion->guardar(); 

                    if($resultado){
                        Ubicacion::setAlerta('exito-ubicacion', 'general', 'El registro ha sido guardado satisfactoriamente');
                    }else{
                        Ubicacion::setAlerta('error-ubicacion', 'general', 'No fue posible guardar el registro');
                    }
                }
            }
        }

        $alertas=Ubicacion::getAlertas();

        $router->renderPanel('panel/ubicacion/index', [
            'ubicacion' => $ubicacion,
            'alertas' => $alertas
        ]);
    }


    public static function editar(Router $router){

        // 1. Verificar que el usuario haya iniciado sesión y que por el método get haya recibido 
        if(!isset($_SESSION['nombre']) || ($_SERVER['REQUEST_METHOD'] === 'GET' && !isset($_GET['id']))){
            header('Location: /');
        }

        //2. Verificar si hay un elemento get y es un número
        if(!is_numeric($_GET['id'])) return;
        
        //3. Buscar el id de la Marca y cargar el objeto en memoria
        $ubicacion = Ubicacion::find($_GET['id']);

        $alertas=[];

        if ($_SERVER['REQUEST_METHOD'] === 'POST'){

            // 1. Sincronizar lo que digito el usuario en el objeto en memoria
            $ubicacion->sincronizar($_POST);

            // 2. Validamos lo digitado por el usuario
            $alertas = $ubicacion->validar();

            if(empty($alertas)){
                
                // Si el nombre de la descripción no existe en la base de datos se procede a guardar
                $resultado = $ubicacion->guardar();

                if($resultado){
                    Ubicacion::setAlerta('exito-ubicacion', 'general', 'El registro ha sido guardado satisfactoriamente');
                }else{
                    Ubicacion::setAlerta('error-ubicacion', 'general', 'No fue posible guardar el registro. Posiblemente ya exista un registro con la misma descripción o no hay conexión con la base de datos');
                }
            }
            $alertas=Ubicacion::getAlertas();
        }

        //4. Verificamos si realiza algún cambio en el método POST
        $router->renderPanel('panel/ubicacion/editar',[
            'ubicacion' => $ubicacion,
            'alertas' => $alertas
        ]);
    }

    
    public static function estado(){

        if(!isset($_SESSION['nombre'])){
            header('Location: /');
        }

        $ubicacion = new Ubicacion($_POST);
        $resultado = $ubicacion->guardar();

        echo json_encode($resultado);
    }

    public static function eliminar(){

        if(!isset($_SESSION['nombre'])){
            header('Location: /');
        }

        $ubicacion = new Ubicacion($_POST);
        $resultado = $ubicacion->eliminar($ubicacion->Ubicacion_Id);

        echo json_encode($resultado);
    }
}

?>